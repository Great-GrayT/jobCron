    ...
    return (
      <section className="panel">
        <div className="row" style={{ justifyContent: "space-between" }}>
          ...
          <button type="button" className="btn ghost sm btn-goo" onClick={() => setDraft(null)}><X size={14} /> Cancel</button>
        </div>
        <p className="hint">
        ...
        <div className="jfs-builder">
            ...
            return (
              <Fragment key={i}>
                <div className="jfs-cond">
                  ...
                  <button type="button" className="btn danger sm btn-press" onClick={() => removeCond(i)} aria-label="remove condition" disabled={draft.conditions.length === 1}>
                    <Trash2 size={14} />
                  </button>
                </div>
          ...
          <button type="button" className="btn ghost sm btn-goo" onClick={addCond}><Plus size={14} /> add condition</button>
        </div>
        ...
        <div className="row" style={{ marginTop: "1rem" }}>
          ...
          <button type="button" className={`btn btn-sub ${saving ? "is-loading" : ""}`} onClick={save} disabled={!valid || saving}>
  ...
  return (
    <section className="panel">
      <div className="row" style={{ justifyContent: "space-between" }}>
        ...
        <button type="button" className="btn btn-flourish" onClick={newSet}><Plus size={16} /> New filter set</button>
      </div>
      <p className="hint">
          ...
          {sets.map((fs) => (
            <div key={fs.id} className={`jfs-set-card ${fs.enabled ? "" : "paused"}`}>
              ...
              <div className="cell-actions cell-actions-end">
                <button type="button" className="btn ghost sm btn-goo" disabled={busy === fs.id} onClick={() => togglePause(fs)}>
                  {fs.enabled ? <><Pause size={13} /> pause</> : <><Play size={13} /> resume</>}
                </button>
                <button type="button" className="btn sm btn-goo-b1" onClick={() => editSet(fs)}><Pencil size={13} /> edit</button>
                <button type="button" className="btn danger sm btn-press" disabled={busy === fs.id} onClick={() => remove(fs)}><Trash2 size={13} /></button>
              </div>
            </div>
          ))}
        </div>
