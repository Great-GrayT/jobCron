...
function RssAppInner() {
  ...
  const actions = (
    <>
      <Link href="/dashboard/schedules" className="button is-light is-small"><Clock size={14} /> Edit schedule</Link>
      <button className="button is-primary is-small" onClick={callAll} disabled={!!busy}>
              ...
              <div key={f.id}>
                <div className="run-row">
                  ...
                  <div className="cell-actions" style={{ marginLeft: "auto" }}>
                    <button className="button is-light is-small" disabled={busy === `test:${f.id}`} onClick={() => run(`test:${f.id}`, () => feedsApi.test(f.id))}><Plug size={13} /> test</button>
                    <button className="button is-primary is-small" disabled={busy === `send:${f.id}`} onClick={() => run(`send:${f.id}`, () => feedsApi.send(f.id))}><Send size={13} /> call</button>
                  </div>
                </div>
