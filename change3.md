...
const [loadedJobsById, setLoadedJobsById] = useState<Map<string, JobStatistic>>(
  new Map(),
);
const [descriptionCache, setDescriptionCache] = useState<Map<string, string>>(
  new Map(),
);
const [loadingDescriptionsDate, setLoadingDescriptionsDate] = useState<
  string | null
>(null);
```
...
useEffect(() => {
  if (loadingBarRef.current)
    loadingBarRef.current.style.width = `${loadingProgress}%`;
  if (jobsBarRef.current)
    jobsBarRef.current.style.width = `${loadingProgress}%`;
}, [loadingProgress]);
```
